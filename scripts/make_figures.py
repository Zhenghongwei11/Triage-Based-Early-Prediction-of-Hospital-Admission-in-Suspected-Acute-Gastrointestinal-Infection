...
def make_auc_by_year(summary_tsv: Path, out_png: Path) -> None:
    ...
    if len(pooled):
        p = pooled.iloc[0]
        plt.axhline(float(p["auc"]), linestyle="--", linewidth=1.5, label=f"Pooled {p['run_id']} AUC")
...
def make_calibration_plot(calib_bins_tsv: Path, *, dev_id: str, ext_id: str, out_png: Path) -> None:
    ...
    plot_one(dev_id, f"{dev_id} (internal OOF)")
    plot_one(ext_id, f"{ext_id} (external)")
    plt.xlim(0, 1)
    plt.ylim(0, 1)
...
def make_dca_plot(decision_curve_tsv: Path, *, dataset_id: str, out_png: Path) -> None:
    ...
    plt.title(f"Decision curve analysis (external): {dataset_id}")
    plt.grid(True, alpha=0.3)
    plt.legend(frameon=False)
    out_png.parent.mkdir(parents=True, exist_ok=True)
    plt.tight_layout()
...
def main() -> None:
    ...
    make_auc_by_year(args.summary, args.outdir / "auc_by_year.png")

    calib_bins = args.pooled_run_dir / "benchmarks/calibration_bins.tsv"
    if calib_bins.exists():
        make_calibration_plot(calib_bins, dev_id=args.dev_id, ext_id=args.ext_id, out_png=args.outdir / "calibration_binned_pooled.png")

    dca = args.pooled_run_dir / "benchmarks/decision_curve.tsv"
    if dca.exists():
        make_dca_plot(dca, dataset_id=args.ext_id, out_png=args.outdir / "dca_external_pooled.png")
